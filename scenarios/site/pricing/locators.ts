...
export const SUBSCRIPTION_ADDRESS = 'mailto:subscription@openbraininstitute.org';

export function pricingLocators(page: Page) {
  const card = (plan: Plan) => page.getByTestId(`pricing-plan-${plan.toLowerCase()}`);
  ...
  return {
    ...
    card,
    planCards: page.getByTestId(/^pricing-plan-(free|pro|enterprise|education)$/),
    contactUs: (plan: Plan) => page.getByTestId(`pricing-plan-${plan.toLowerCase()}-contact`),
  };
}
