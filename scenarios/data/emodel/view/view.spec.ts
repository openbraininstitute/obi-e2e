...
const SLUG = entitySlug(Type.Emodel);
...
test.describe('E-model details', () => {
  ...
  test('Open one E-model beside the listing', { tag: PRIVATE_READONLY }, async ({ page }) => {
  ...
  test('Open the full E-model page', { tag: PRIVATE_READONLY }, async ({ page }) => {
    test.slow();
    const view = dataView(page);
