...
const SLUG = entitySlug(Type.MemodelCircuitSimulation);
...
test.describe('Single neuron details', () => {
  ...
  test('Open one Single neuron beside the listing', { tag: PRIVATE_READONLY }, async ({ page }) => {
  ...
  test('Open the full Single neuron page', { tag: PRIVATE_READONLY }, async ({ page }) => {
    test.slow();
    const view = dataView(page);
