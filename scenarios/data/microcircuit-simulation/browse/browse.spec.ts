...
test.describe('Microcircuit listing', () => {
  test.beforeEach(async ({ page, workspace }) => {
    await page.goto(
      routes.dataEntity(
        workspace.labId,
        workspace.projectId,
        entitySlug(Type.MicrocircuitSimulation)
      )
    );
    await expect(entityListing(page).table).toBeVisible();
  });

  test('See the Microcircuit table', { tag: PRIVATE_READONLY }, async ({ page }) => {
  ...
  test('See the Microcircuit results', { tag: PRIVATE_READONLY }, async ({ page }) => {
    const listing = entityListing(page);
  ...
  test('Search the Microcircuit listing', { tag: PRIVATE_READONLY }, async ({ page }) => {
    const listing = entityListing(page);
    await expect(listing.cells.first()).toBeVisible();
