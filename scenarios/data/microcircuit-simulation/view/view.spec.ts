...
const SLUG = entitySlug(Type.MicrocircuitSimulation);
...
test.describe('Microcircuit details', () => {
  ...
  test('Open one Microcircuit beside the listing', { tag: PRIVATE_READONLY }, async ({ page }) => {
  ...
  test('Open the full Microcircuit page', { tag: PRIVATE_READONLY }, async ({ page }) => {
    test.slow();
    const view = dataView(page);
