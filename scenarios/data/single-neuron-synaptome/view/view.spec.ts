...
const SLUG = entitySlug(Type.SingleNeuronSynaptome);
...
test.describe('Synaptome (legacy) details', () => {
  ...
  test('Open the full Synaptome (legacy) page', { tag: PRIVATE_READONLY }, async ({ page }) => {
    test.slow();
    const view = dataView(page);
