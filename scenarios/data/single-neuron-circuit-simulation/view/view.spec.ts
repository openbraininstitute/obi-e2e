...
const SLUG = entitySlug(Type.SingleNeuronCircuitSimulation);
...
test.describe('Synaptome simulation details', () => {
  test.beforeEach(async ({ page, workspace }) => {
    ...
    test.skip(empty.startsWith('0 results'), 'This deployment holds no Synaptome to open.');

    await listing.cells.filter({ hasText: /\S/ }).first().click();
    await expect(dataView(page).viewDetails).toBeVisible();
  });

  test('Open one Synaptome beside the listing', { tag: PRIVATE_READONLY }, async ({ page }) => {
  ...
  test('Open the full Synaptome page', { tag: PRIVATE_READONLY }, async ({ page }) => {
    test.slow();
    const view = dataView(page);
