...
const SLUG = entitySlug(Type.PairedNeuronCircuitSimulation);
...
test.describe('Paired neurons details', () => {
  test.beforeEach(async ({ page, workspace }) => {
    ...
    test.skip(empty.startsWith('0 results'), 'This deployment holds no Paired neurons to open.');

    await listing.cells.filter({ hasText: /\S/ }).first().click();
    await expect(dataView(page).viewDetails).toBeVisible();
  });

  test('Open one Paired neurons beside the listing', { tag: AUTHENTICATED }, async ({ page }) => {
  ...
  test('Open the full Paired neurons page', { tag: AUTHENTICATED }, async ({ page }) => {
    test.slow();
    const view = dataView(page);
