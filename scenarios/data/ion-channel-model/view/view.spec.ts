...
const SLUG = entitySlug(Type.IonChannelModel);
...
test.describe('Ion channel model details', () => {
  ...
  test(
    'Open one Ion channel model beside the listing',
  ...
  test('Open the full Ion channel model page', { tag: PRIVATE_READONLY }, async ({ page }) => {
    test.slow();
    const view = dataView(page);
