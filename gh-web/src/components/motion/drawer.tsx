...
export interface DrawerProps {
  ...
  dismissable?: boolean;
  /**
   * LOCAL ADDITION, not in the beUI registry version. False renders no backdrop
   * and leaves the page scrollable, so a drawer can sit beside the page instead
   * of over it. The backdrop cannot be neutralised from the outside: the
   * presence gate sets `pointer-events: auto` inline, which beats any class
   * passed through `backdropClassName`. See gh-web/README.md.
   */
  modal?: boolean;
}
...
export function Drawer({
  ...
  useEffect(() => {
    ...
    // A drawer that sits beside the page must not freeze it.
    if (!modal) return () => window.removeEventListener('keydown', onKey);
